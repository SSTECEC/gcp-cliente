...
@Component({
  selector: 'app-responsable-reserva',
  templateUrl: './responsable-reserva.component.html',
  styleUrls: ['./responsable-reserva.component.css']
})
export class ResponsableReservaComponent implements OnInit {
  ...
  public crearReserva(reservaSeleccionada:any){
    this.connection.get("actualizarReserva?idUsuario=" + this.usuario.idUsuario + "&idReserva="+ reservaSeleccionada.idReserva, "").subscribe(
      (res: any) => {
        this.spinner.hide();
        if(res){
          this.globales.alertaSinTiempo("Transacción Exitosa","Reserva Creada Exitosamente","success");
          this.listadoCompletoReservaciones();
        }
      },
      err => {
