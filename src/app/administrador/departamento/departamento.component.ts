...
export class DepartamentoComponent implements OnInit {
  ...
  public listarRegistrosDepartamento() {
    this.spinner.show();
    this.connection.get("listarRegistrosDepartamento", "").subscribe(
      (res: any) => {

        var codigo = parseInt(res.total) + 1;
        var numeroCotizacion = "0";
        switch (codigo.toString().length) {
          case 1: numeroCotizacion = "000" + codigo; break;
          case 2: numeroCotizacion = "00" + codigo; break;
          case 3: numeroCotizacion = "0" + codigo; break;
          default: numeroCotizacion = codigo.toString();
        }
        this.formulario.codigo = numeroCotizacion;
        this.spinner.hide();
  ...
  public listarSedeDetallesParametro() {
    this.lstSedeDetallesParametro = [];
    this.spinner.show();
    this.connection.get("listarSedeDetallesParametro?idSede=" + this.formulario.idSede, "").subscribe(
      (res: any) => {
        this.spinner.hide();
        if (res.length != 0) {
          this.lstSedeDetallesParametro = res;
        } else {
          this.globales.alertaSinTiempo("Información", "La sede seleccionada no cuenta con secciones disponibles <br> <b>Elija otra sede.</b> ", "info");
        }
      },
      err => {
        console.log(err);
  ...
  public guardarDepartamento() {
    if (this.globales.validarModalGuardarDepartamento(this.formulario)) {
      this.spinner.show();
      this.connection.post("guardarDepartamento", this.formulario, "").subscribe(
        (res: any) => {
          this.spinner.hide();
          this.limpiar();
