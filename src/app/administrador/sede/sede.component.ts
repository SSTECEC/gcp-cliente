...
export class SedeComponent implements OnInit {
  ...
  public guardarSede() {
    if (this.globales.validarModalCrearSede(this.fmrSede, this.fmrSedeDetalle)) {
      this.spinner.show();
  ...
  public guardarSedeDetalles(idSede: any) {
    ...
    var candena = "";
    var piso = 1;
    for (let j = 0; j < parseInt(this.fmrSedeDetalle.pisos); j++) {
      var aux1 = (piso++);
      var seccion = 1;
    ...
    var query = {
      cadena: candena.substr(0, (candena.length - 1))
    }

    this.connection.post("guardarSedeDetalles", JSON.stringify(query), "").subscribe(
      (res: any) => {
        this.listarSedes();
        location.reload();
        this.globales.notificacion("Detalles almacenadados exitosamente", "success", "top");
      },
      err => {
        console.log(err);
        this.globales.notificacion("Error con el servicio de datos", "error", "top");
  ...
  public verDetallesSede(sede: any) {

    this.connection.get("listarSedeDetallesParametro?idSede=" + sede.idSede, "").subscribe(
      (res: any) => {
  ...
  public abrirModalEditarSede(sede: any) {
    this.fmrSede = sede;
