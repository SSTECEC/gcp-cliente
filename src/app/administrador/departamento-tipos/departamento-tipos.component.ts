...
@Component({
  selector: 'app-departamento-tipos',
  templateUrl: './departamento-tipos.component.html',
  styleUrls: ['./departamento-tipos.component.css']
})
export class DepartamentoTiposComponent implements OnInit {
  ...
  public guardarTipo() {
    if (this.globales.validarModalCrearTipoDepartamento(this.formulario)) {

      this.spinner.show();
      this.connection.post("guardarTipoDepartamento", this.formulario, "").subscribe(
        (res: any) => {
          this.spinner.hide();
          $("#ModalTipo").modal("toggle");
          this.listarDepartamentos();
          this.globales.notificacion("Tipo de departamento almacenado exitosamente", "success", "top");
        },
        err => {
          console.log(err);
          this.spinner.hide();
