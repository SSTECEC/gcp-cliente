...
export class GestionReservaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.fechaFinalizacion = moment().format('YYYY-MM-DD');
    ...
    this.connection.get("listarUltimaFechaReserva", "").subscribe(
      (res: any) => {
        ...
        if(res == null){
          ...
          console.log('Sin reservas aún', this.fechaInicioMin );
        }else {
          this.fechaInicioMin = moment(res.ultimaFecha).format('YYYY-MM-DD');
          console.log('Con reservas ya', this.fechaInicioMin);
        }
      },
      err => {
        console.log(err);
    ...
    if (this.fechaInicio != undefined && this.fechaFinalizacion != undefined) {
      ...
      while (rangoFechaFin.getTime() >= rangoFechaInicio.getTime()) {
        ...
        for (var aula of this.listadoAulas) {
          for (let i = 1; i < 16; i++) {
            datos += "(" + aula.idDepartamento + "," + this.usuario.idUsuario + ",1,'" + fechaTotal + "'," + horaInicio + "," + horaFin + "),";
            horaInicio++;
            horaFin++;
          }
          horaInicio = 7;
