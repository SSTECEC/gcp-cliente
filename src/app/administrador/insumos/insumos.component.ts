import { Component, OnInit, ViewChild } from '@angular/core';
import { NgxSpinnerService } from 'ngx-spinner';
import { GlobalsService } from '../../method/globals/globals.service';
import { ConnectionService } from '../../service/connection/connection.service';
import { SessionService } from '../../service/session/session.service';
import { process, State } from '@progress/kendo-data-query';
import { GridComponent, GridDataResult, DataStateChangeEvent, DataBindingDirective } from '@progress/kendo-angular-grid';

declare var $: any;
@Component({
  selector: 'app-insumos',
  templateUrl: './insumos.component.html',
  styleUrls: ['./insumos.component.css']
})
export class InsumosComponent implements OnInit {

  @ViewChild(DataBindingDirective) dataBinding: DataBindingDirective;
  public gridData: any[];
  public gridView: any[];


  public usuario: any = { "idUsuario": 1, "usuario": "Gabriela Andrango", "email": "gaby@gmail.com", "contrasena": "12345", "foto": "data:image/png;base64,", "idRol": 1, "Rol": "ADMINISTRADOR" };

  public lstDatos = [];

  public formulario = {
    nombre: '',
    caracteristicas: '',
    cantidad: '',
    identificador: '',
    numeroSerie: 1,
    idInsumo: 0
  }

  public numeroSerieTotal: any;


  public lstDatosDetalleInsumos = [];
  public dtsDatosDetalleInsumos: any;
  /* 
  public state: State = { skip: 0, take: 5 };
  
  public gridData: GridDataResult = process(this.lstDatosDetalleInsumos, this.state);
  public gridView: any[]; */

  public globales: GlobalsService = new GlobalsService();
  constructor(public session: SessionService, private spinner: NgxSpinnerService, public connection: ConnectionService) { }

  ngOnInit(): void {
    this.usuario = this.session.obtenerDatos();
    this.listarInsumos();
  }

  public listarInsumos() {
    this.spinner.show();
    this.connection.get("listarInsumos", "").subscribe(
      (res: any) => {
        this.spinner.hide();
        console.log(res);
        this.lstDatos = res;
      },
      err => {
        console.log(err);
        this.spinner.hide();
        this.globales.notificacion("Error con el servicio de datos", "error", "top");
      }
    );
  }

  public guardarInsumos() {

    var ident = this.formulario.nombre.substring(0, 3);

    var datos = {
      nombre: this.formulario.nombre,
      caracteristicas: this.formulario.caracteristicas,
      cantidad: this.formulario.cantidad,
      identificador: ident,
      numeroSerie: 1,
      disponibles: this.formulario.cantidad,
      asignados: 0
    }

    if (this.globales.validarModalGuardarInsumo(this.formulario)) {
      this.spinner.show();
      this.connection.post("guardarInsumos", datos, "").subscribe(
        (res: any) => {
          if (res) {
            this.numeroSerieTotal = res.numeroSerie + parseInt(datos.cantidad);
            this.generarDetalleInsumo(res.idInsumo, datos.identificador, res.numeroSerie, datos.cantidad);
            this.spinner.hide();
          }
        },
        err => {
          console.log(err);
          this.spinner.hide();
          this.globales.notificacion("Error con el servicio de datos", "error", "top");
        },
      );
    }

  }

  public generarDetalleInsumo(idInsumo: any, identificador: any, numeroSerie: any, cantidad: any) {

    var cadena = '';

    for (let i = 0; i < parseInt(cantidad); i++) {
      cadena += "(" + idInsumo + ", '" + this.generarCodigoCotizacion(identificador, numeroSerie) + "'," + "1,1" + "),";
      numeroSerie++
    }

    var query = {
      cadenaFinal: cadena.substr(0, (cadena.length - 1))
    }

    this.guadarDetalleInsumo(idInsumo, query);

  }

  public guadarDetalleInsumo(idInsumo: any, query: any) {
    console.log('res')
    this.spinner.show();
    this.connection.post("guardarInsumoDetalles", JSON.stringify(query), "").subscribe(
      (res: any) => {
        if (res) {
          this.actualizarNumeroSerie(idInsumo, this.numeroSerieTotal);
          this.spinner.hide();
        }
      },
      err => {
        console.log(err);
        this.spinner.hide();
        this.globales.notificacion("Error con el servicio de datos", "error", "top");
      },
    );
  }

  public actualizarNumeroSerie(idInsumo: any, numeroSerieFinal: any) {
    var datos: any = {
      "idInsumo": idInsumo,
      "numeroSerie": numeroSerieFinal
    }
    this.spinner.show();
    this.connection.post("actualizarNumeroSerieInsumo", datos, "").subscribe(
      (res: any) => {
        if (res) {
          $("#ModalCrear").modal("toggle");
          location.reload();
          this.listarInsumos();
          this.globales.notificacion("Tipo de Insumo almacenado exitosamente", "success", "top");
          this.spinner.hide();
        }
      },
      err => {
        console.log(err);
        this.spinner.hide();
        this.globales.notificacion("Error con el servicio de datos", "error", "top");
      },
    );
  }

  public eliminarInsumo(id: any) {

    var datos: any = {
      'idInsumo': id
    }

    this.spinner.show();
    this.connection.post("eliminarInsumos", datos, "").subscribe(
      (res: any) => {
        if (res) {
          this.globales.notificacion("Insumo Eliminado Exitosamente", "success", "top");
        }
        this.spinner.hide();
      },
      err => {
        console.log(err);
        this.spinner.hide();
        this.globales.notificacion("Error con el servicio de datos", "error", "top");
      }
    );
  }

  public verdetallesInsumos() {
    $("#ModalTipo").modal("show");

  }

  public listarDetalleInsumos(insumoSeleccionado: any) {

    var idInsumo = insumoSeleccionado;

    this.spinner.show();
    this.connection.get("listarDetalleInsumos?idInsumo=" + idInsumo, " ").subscribe(
      (res: any) => {
        this.spinner.hide();
        this.lstDatosDetalleInsumos = res;
        this.dtsDatosDetalleInsumos = this.lstDatosDetalleInsumos.slice()
        this.gridData = this.lstDatosDetalleInsumos;
        this.gridView = this.gridData;
        $("#ModalTipo").modal("show");
      },
      err => {
        console.log(err);
        this.spinner.hide();
        this.globales.notificacion("Error con el servicio de datos", "error", "top");
      }
    );

  }

  public actualizarEstadoDetalleInsumo(detalleInsumoSeleccionado: any) {


    console.log('detalleInsumoSeleccionado', detalleInsumoSeleccionado)

    var estadoDetalle: any;

    if (detalleInsumoSeleccionado.estado == 1) {
      estadoDetalle = 2;
    } else {
      estadoDetalle = 1;
    }

    var datos: any = {
      idDetalleInsumo: detalleInsumoSeleccionado.idDetalleInsumo,
      estado: estadoDetalle
    }

    this.spinner.show();
    this.connection.post("cambiarEstadoDetalleInsumo", datos, "").subscribe(
      (res: any) => {
        if (res) {
          this.spinner.hide();
          this.globales.notificacion("Estado cambiado exitosamente", "success", "top");
          this.listarDetalleInsumos(detalleInsumoSeleccionado.idInsumo);
        }
      },
      err => {
        console.log(err);
        this.spinner.hide();
        this.globales.notificacion("Error con el servicio de datos", "error", "top");
      },
    );

  }

  public generarCodigoCotizacion(identificador: any, numero: any) {
    var inicio = identificador + '-';
    var fecha = this.obtenerFecha("");
    var numeroCotizacion = "0";

    switch (numero.toString().length) {
      case 1:
        numeroCotizacion = "000" + numero;
        break;
      case 2:
        numeroCotizacion = "00" + numero;
        break;
      case 3:
        numeroCotizacion = "0" + numero;
        break;
      default:
        numeroCotizacion = numero;
    }
    var codigoCertificado = inicio + fecha + "-" + numeroCotizacion;

    return codigoCertificado;

  }

  public obtenerFecha(separador: any) {
    var fecha = new Date();
    var anio = fecha.getFullYear();
    var mes: any = fecha.getMonth() + 1;
    var dia: any = fecha.getDate();

    mes < 10 ? mes = "0" + mes : mes;
    dia < 10 ? dia = "0" + dia : dia;

    return anio + separador + mes + separador + dia;
  }


  public onFilter(inputValue: string): void {
    this.gridView = process(this.gridData, {
      filter: {
        logic: "or",
        filters: [
          {
            field: 'codigo',
            operator: 'contains',
            value: inputValue
          }
        ],
      }
    }).data;

    this.dataBinding.skip = 0;
  }


  public abrirModalEditarInsumo(dato: any) {
    console.log(dato);
    this.formulario = dato;

    console.log(this.formulario);
    $("#EditarInsumo").modal("show");
  }

  public abrirModalCrearInsumo() {
    this.limpiarInsumo();
    console.log('abrirModalCrearInsumo', this.formulario);
    $("#ModalCrear").modal("show");
  }

  public limpiarInsumo() {
    this.formulario = {
      nombre: '',
      caracteristicas: '',
      cantidad: '',
      identificador: '',
      numeroSerie: 1,
      idInsumo: 0
    }
  }

  public actualizarInsumo() {
    if (this.globales.validarModalActualizarInsumo(this.formulario)) {
      var datos: any = {
        'nombre': this.formulario.nombre,
        'caracteristicas': this.formulario.caracteristicas,
        'idInsumo': this.formulario.idInsumo
      }

      this.spinner.show();
      this.connection.post("actualizarInsumos", datos, "").subscribe(
        (res: any) => {
          console.log(res);
          if (res) {
            this.globales.notificacion("Insumo Actualizado Exitosamente", "success", "top");
          }
          this.spinner.hide();
          location.reload();
        },
        err => {
          console.log(err);
          this.spinner.hide();
          this.globales.notificacion("Error con el servicio de datos", "error", "top");
        }
      );
    }
  }

}
