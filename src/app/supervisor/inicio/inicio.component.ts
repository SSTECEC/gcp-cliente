...
export class InicioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.listarReservasTotalesJson();
  }

  public listarReservasTotalesJson() {
    this.spinner.show();
    this.connection.get("listarReservasTotalesJson", "").subscribe(
      (res: any) => {
        ...
        if (res.length != 0) {
          ...
          for (let dato of res) {
            dataSet.x.push(dato.listado.f1);
  ...
  public listarReservasUsuariosJson() {
    this.spinner.show();
    this.connection.get("listarReservasUsuariosJson", "").subscribe(
      (res: any) => {
        this.spinner.hide();
        if (res.length != 0) {
