...
@Component({
  selector: 'app-supervisor-rev-list',
  templateUrl: './supervisor-rev-list.component.html',
  styleUrls: ['./supervisor-rev-list.component.css']
})
export class SupervisorRevListComponent implements OnInit {
  ...
  public listarPagoParametros() {
    var cadena = ' "idCatalogoEstado"=2 AND "idUsuario" = ' + this.usuario.idUsuario + ' ';
  ...
  public cancelarReserva(reservaSeleccionada:any){
    this.connection.get("cancelarReserva?idUsuario=" + this.usuario.idUsuario + "&idReserva="+ reservaSeleccionada.idReserva, "").subscribe(
      (res: any) => {
        this.spinner.hide();
        if(res){
          this.globales.alertaSinTiempo("Transacción Exitosa","Reserva Cancelada Exitosamente","success");
          this.listadoCompletoReservacionesUsuario();
        }
      },
      err => {
